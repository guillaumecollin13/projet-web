<form method="post" action="">
    ...
    <fieldset>
        ...
        <div class="mb-3">
            ...
            <input type="text" placeholder="Nom" class="form-control" id="Nom" name="nom" value="<?php if(isset($_SESSION["user"])) : ?><?= $_SESSION["user"] ->userNom ?><?php else :?>coucou<?php endif?>">
        ...
        <div class="mb-3">
            ...
            <input type="text" placeholder="pseudonyme" class="form-control" id="pseudo" name="pseudo" value="<?php if(isset($_SESSION["user"])) : ?><?= $_SESSION["user"] ->userPseudo ?><?php endif?>">
            <?php if(isset($messageErreur["pseudo"])): ?> <small><?= $messageErreur["pseudo"]?></small><?php endif?>
        </div>
        <div class="mb-3">
            <label for="email" class="form-label">Email</label>
            <input type="<?php if(isset($_SESSION["user"])) : ?>disabled <?php else : ?>email<?php endif?>" placeholder="Email" class="form-control" id="email" name="email" value="<?php if(isset($_SESSION["user"])) : ?><?= $_SESSION["user"] ->userMail ?><?php endif?>">
